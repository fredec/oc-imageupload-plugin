 ...
 */
 +function ($) { "use strict";
    ...
    FileUpload.prototype.init = function(target) {
        ...
        this.$uploadButton = $('.fileupload-button', this.$el)
    ...
    FileUpload.prototype.onUploadSuccess = function(file, response) {
         ...
         */
         ...
         $preview.append('<input type="hidden" name="'+response.name_file+'[]" value="'+response.id+'" />');
     }
    ...
    FileUpload.DEFAULTS = {
        ...
        uniqueId: null,
        extraData: {},
        paramName: 'Fileupload_file_data',
        fileTypes: null,
        template: null,
        isMulti: null,
        isPreview: null,
    ...
    if($('*[data-control="fileupload_uploads"]').length > 0){
        FileUpload.custom=[];
        $('*[data-control="fileupload_uploads"]').each(function(){
            var name_=$(this).attr('data-paramName');
            if(FileUpload.custom[name_] == undefined){
                FileUpload.custom[name_] = {
                    url: window.location,
                    uniqueId: null,
                    extraData: {},
                    paramName: name_,
    ...
    $.fn.fileUploader = function (option) {
        return this.each(function () {
            var configs_=FileUpload.custom[$(this).attr('data-paramName')];
            var $this   = $(this)
            var data    = $this.data('oc.fileUpload')
            var options = $.extend({}, configs_, $this.data(), typeof option == 'object' && option)
            // $this.accept = "image/*";
    ...
    $(document).ready(function(){
        $(document).render(function () {
            $('[data-control="fileupload_uploads"]').fileUploader();
        })
        $('form').bind('reset', function() {
            $(this).find('.upload-files-container').html('');
        });
    })
