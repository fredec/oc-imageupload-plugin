...
class Plugin extends PluginBase
{
	...
	// }
		...
		Event::listen( 'media.file.upload', function ( $widget, $filePath, $uploadedFile ) {
			...
			// $url=$filePath;
			// $url2=str_replace($name[0].'.'.$name[1], str::slug($name[0]).'.'.$name[1], $filePath);

			// // $texto=$url.' - '.$url2.' - '.$filePath;
			...
			// 	if($url_ != $url2_){
			// 		if(!MediaLibrary::instance()->moveFile(str_replace('%20', ' ', $url_),$url2_)){
			// 			$complete_url=false;

			// 			$url_=MediaLibrary::url($filePath);
			// 			$url2_=MediaLibrary::url(str_replace($name[0].'.'.$name[1], str::slug($name[0]).'.'.$name[1], $filePath));
			// 			$url_=implode('/',array_filter(explode('/', $url_)));
			// 			$url2_=implode('/',array_filter(explode('/', $url2_)));

			// 			if(FileHelper::copy(str_replace('%20', ' ', $url_), $url2_)) FileHelper::delete(str_replace('%20', ' ', $url_));
			// 			else rename(str_replace('%20', ' ', $url_), $url2_);
			// 		}
			// 	}
			// 	$url=$url2;
			// 	$info=pathinfo($url);
			...
			// }
			// if($complete_url) $url=MediaLibrary::url($url);
